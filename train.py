...
def training_loop(model,
    ...
    for epoch in range(n_epoch, config.epochs):
        ...
        # Create a tqdm progress bar for the training loop (for each batch in the epoch)
        total_loss = 0
        total_grad_norm = 0
        num_batches = 0

        # Wrap the train_loader with tqdm to get a progress bar
        train_loader_tqdm = tqdm(train_loader, desc=f"Training Epoch {epoch}/{config.epochs}", dynamic_ncols=True)
        ...
        for batch in train_loader_tqdm:
            ...
            # Accumulate the loss and grad_norm for each batch
            total_loss += loss.item()
            total_grad_norm += grad_norm.item()
            num_batches += 1

            writer.add_training_data({'loss': loss, 'grad_norm': grad_norm}, config.learning_rate, n_iter, n_iter)
            ...
            # Update the tqdm progress bar description with current loss and grad_norm
            train_loader_tqdm.set_postfix(loss=total_loss / num_batches, grad_norm=total_grad_norm / num_batches)

        # After finishing the epoch, print the average loss and grad_norm
        avg_loss = total_loss / num_batches
        avg_grad_norm = total_grad_norm / num_batches
        print(f"Epoch {epoch} - Average Loss: {avg_loss:.4f}, Average Grad Norm: {avg_grad_norm:.4f}")
